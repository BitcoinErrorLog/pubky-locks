...
Also this proposal does not specify the type of secure communication between Viewer App and Lock Service.
...
From the traditional web point of view it is a server which may or may not have a user interface.
...
Optionally it may have up to `[/guarded/:rw,/pub/locks.app/:rw]` in case locks are to be created through it.
...
2. Viewer gets "unlock conditions" by reading public `/pub/locks.app/<lock_id>` from the homeserver (alternatively requires one more read of the `/pub/locks.app/config.json` or more elaborate pkarr record parsing)
3. Viewer "solves the lock"
4. Viewer "provides the proof bundle" to `PUT <z32 public key of the lock server>/<creator_id>/unlock/<lock_id>` (from step 2)
