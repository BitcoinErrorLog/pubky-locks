...
3.a Alternatively, lock service url can be stored in  `/pub/locks.app/config.json`:
```json
{
    "locks_service_url": "<z32 public key of the lock server>/<user_id>/unlock/<lock_id>"
}
```
3.b More crazy alternative is to store in user's pkarr record as:
```
...
```
// blah
URL: `<homerver>/pub/locks.app/<lock_id>`
```

##### Diagram 5.1 (simplified)
...
sequenceDiagram
  ...
  Note over C,H: Step 3 — Create lock policy (see 6.1)
...
4. Reader "provides the proof bundle" to `PUT <z32 public key of the lock server>/<user_id>/unlock/<lock_id>` (from step 2)
5. Reader polls verification status `GET <z32 public key of the lock server>/<user_id>/unlock/<lock_id>`
6. Lock server:
  6.1 reads lock conditions `<user_id>/pub/locks.app/<lock_id>`
  6.2 "verifies the proof"
  6.3 In case of success it creates "unlock URL" `<z32 public key of the lock server>/view/<lock_id>` and shares it with reader
...
sequenceDiagram
  ...
  V->>H: GET /pub/<app-id>/posts/<id>
  H-->>V: preview + lock link

  Note over V,H: Step 2: Get unlock conditions
  V->>L: GET /pub/locks.app/<lock_id>
  L-->>V: LockPolicy JSON (see 6.1)

  Note over V,V: Step 3: Lock specific process
  V->>V: unlock

  Note over V,L: Step 4: Proof bundle submission (6.2)
  V->>L: PUT <z32 public key of the lock server>/<user_id>/unlock/<lock_id>
  L-->>V: {task_id, status: "pending"}

  Note over V,L: Async polling
  loop Until eligible or failed
    Note over V,L: Step 5: check status
    V->>L: GET <z32 public key of the lock server>/<user_id>/unlock/<lock_id>
    L-->>V: {task_id, status: "in progress"}

    Note over L,L: Step 6: Lock specific verification process
    L-->>H: Get lock conditions
    L-->>L: Verify unlocks <br/> use 3rd party if necessary
  end

  Note over V,L: Last poll
  V->>L: GET <z32 public key of the lock server>/<user_id>/unlock/<lock_id>
  Note over L,V: Step 6.3: create proxy read url
  L-->>V: {<br/>task_id,<br/> status: "completed", <br/>access_url: "<z32 public key of the lock server>/view/<lock_id>"<br/>}

  Note over V,L: Step 7:
  V->>L: GET <z32 public key of the lock server>/view/<lock_id>
...
#### 5.3 Lock service migration (credible exit)
